...
function spmv_ell(data, indices, N, nc, x, y){
  ...
  for(j = 0; j < nc; j++){
    for(i = 0; i < N; i++)
      y[i] += Math.fround(data[j*N+i] * x[indices[j*N+i]]);
  }
}
...
function spmv_dia(data, offset, N, nd, x, y, stride){
  ...
  for(i = 0; i < nd; i++){
    ...
    for(n = istart; n <= iend; n++){
      y[n] += Math.fround(data[i*stride+n-index] * x[n+k]);
    }
  }
}
...
function spmv_csr(row_ptr, csr_col, csr_val, N, nz, x, y){
  ...
  for(var i = 0; i < N; i++){
    ...
    for(var j = row_ptr[i]; j < row_ptr[i+1]; j++){ 
      //temp[0] += Math.fround(csr_val[j] * x[csr_col[j]]);
      temp = Math.fround(temp) + Math.fround(csr_val[j] * x[csr_col[j]]);
    }
    //y[i] = temp[0];
    y[i] = temp;
  }
    ...
    request.onreadystatechange = function() {
      if(request.readyState == 4 && request.status == 200){
        ...
        if(fileno < num)
          ...
          var coo_row = new Int32Array(anz), coo_col = new Int32Array(anz), coo_val = new Float32Array(anz), x = new Float32Array(cols), y = new Float32Array(rows);
          var csr_row = new Int32Array(rows+1), csr_col = new Int32Array(anz), csr_val = new Float32Array(anz), csr_y = new Float32Array(rows);
          var dia_y = new Float32Array(rows);
          var ell_y = new Float32Array(rows);
          var t1, t2, tt = 0.0;
          if(symmetry == "symmetric"){
            if(field == "pattern"){
          ...
          var nd = result[0]
          var stride = result[1]
          if(nd*stride < Math.pow(2,28)){
            var offset = new Int32Array(nd);
            var dia_data = new Float32Array(stride*nd);
          ...
          if(nc*N < Math.pow(2,28)){
            var indices = new Int32Array(nc*N), ell_data = new Float32Array(nc*N);
